function [ meas_data date_new] = SR_fetchC(filename,in_date,out_date)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:length(data_final)-1
 ...
 if (minute(data_final(i,2)) - t15(h)) <= 15 && (minute(data_final(i+1,2)) - t15(h)) >= 0        
        T_var(k) = data_final(i,j+2);
        k = k + 1;
        
 elseif (minute(data_final(i,2)) - t15(h)) > 15
        
        T_new{j}(h) = nanmean(T_var);
...
% Find the starting and end row numbers
    start_row = find(t_new == in_datenum);
    end_row   = find(t_new == out_datenum); 
...
for i = 1:3
    meas_data{i} = T_new{i}(start_row:end_row);
end
