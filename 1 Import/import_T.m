...
for k=1:size(dftest,1)-1
    ...
    if (data_final(k+1,1)+data_final(k+1,2))-(data_final(k,1)+data_final(k,2)) > 1.05*15/60/24  %To avoid rounding erros
...
date=[datestr(data_final(:,1)) datestr(data_final(:,2))];
data=data_final;
header=header_final;
